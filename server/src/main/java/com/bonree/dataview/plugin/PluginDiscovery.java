...
import com.bonree.dataview.demo.Greeting;
...
final class PluginDiscovery {
    ...
    private static final String SERVICES_FILE = Paths.get("META-INF", "services", Greeting.class.getName()).toString();
    private static final String TARGET_CLASSES = Paths.get("/target", "classes").toString();

    private PluginDiscovery() {
    }

    public static Set<String> discoveryPlugins(Artifact artifact, ClassLoader classLoader) throws IOException {
        ...
        return listClasses(file.toPath()).stream()
                                         .filter(name -> classInterfaces(name, classLoader).contains(Greeting.class.getName()))
    ...
    private static List<String> listClasses(Path base) throws IOException {
